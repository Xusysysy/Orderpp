...
import com.opp.order.DbState
import com.opp.order.OrderApp
import com.opp.order.data.db.DatabaseHelper
import com.opp.order.data.repository.MenuRepository
import com.opp.order.data.repository.OrderRepository
import com.opp.order.data.repository.TableRepository
...
import com.opp.order.ui.screen.main.MainScreen
import com.opp.order.ui.theme.OrderTheme
import com.opp.order.util.LogWriter
import com.opp.order.viewmodel.HostViewModel
import com.opp.order.viewmodel.MenuViewModel
import com.opp.order.viewmodel.OrderViewModel
import com.opp.order.viewmodel.RoleViewModel
import com.opp.order.viewmodel.TableViewModel

@Composable
fun OrderAppContent() {
    val app = LocalContext.current.applicationContext as OrderApp
    ...
    Surface(modifier = Modifier.fillMaxSize(), color = Color(0xFF121212)) {
        Box(contentAlignment = Alignment.Center, modifier = Modifier.fillMaxSize()) {
            Column(horizontalAlignment = Alignment.CenterHorizontally) {
                Text("Order++", color = Color(0xFFFFB300), fontSize = 28.sp)
...
private fun ErrorScreen(message: String) {
    ...
    Surface(modifier = Modifier.fillMaxSize(), color = Color(0xFF121212)) {
        Column(
            ...
            Spacer(modifier = Modifier.height(16.dp))
            Text("日志路径: ${ctx.filesDir.absolutePath}/Order_log.txt", color = Color(0xFF4CAF50), fontSize = 12.sp)
        }
    }
}

@Composable
private fun MainApp(helper: DatabaseHelper, app: OrderApp) {
    var isDark by remember { mutableStateOf(true) }

    OrderTheme(darkTheme = isDark) {
        val prefs = remember { app.getSharedPreferences("Order_prefs", Context.MODE_PRIVATE) }
        ...
        val tableDao = remember { com.opp.order.data.db.dao.TableDao(helper) }
        val menuDao = remember { com.opp.order.data.db.dao.MenuItemDao(helper) }
        val recipeDao = remember { com.opp.order.data.db.dao.RecipeDao(helper) }
        val orderDao = remember { com.opp.order.data.db.dao.OrderDao(helper) }
        ...
        hostViewModel.onSyncPin = { pin ->
            com.opp.order.util.PinHelper.setPin(pin)
        }
            ...
            discoveryService.onHostDiscovered = { info ->
                ...
                if (currentRole == RoleViewModel.Role.GUEST) {
                    ...
                    if (ip != null) {
                        ...
                        if (!alreadyConnected) {
                            if (knownHost == name) {
                                val client = com.opp.order.network.SyncClient(ip)
