...
import com.opp.order.data.preset.PresetRecipes
import com.opp.order.util.LogWriter
import com.opp.order.util.PinHelper
...
class OrderApp : Application() {
    ...
    override fun onCreate() {
        ...
        PinHelper.init(this)
        LogWriter.write("APP", "OrderApp v1.0.1 onCreate start")
        appScope.launch {
            try {
                ...
                LogWriter.write("APP", "OrderApp init SUCCESS")
            } catch (e: Exception) {
                val msg = "${e.javaClass.simpleName}: ${e.message}"
                LogWriter.write("APP", "OrderApp init FAILED", e)
        ...
        try {
            ...
            for ((index, c) in PresetRecipes.cocktails.withIndex()) {
                val menuId = menuDao.insert(
                    com.opp.order.data.db.entity.MenuItemEntity(
                ...
                for ((i, step) in c.steps.withIndex()) {
                    recipeDao.insertStep(
                        com.opp.order.data.db.entity.RecipeStepEntity(
                ...
                for ((name, amount) in c.ingredients) {
                    ...
                    recipeDao.insertIngredient(
                        com.opp.order.data.db.entity.RecipeIngredientEntity(
            ...
            for ((name, cat, price) in PresetRecipes.defaultDrinks) {
                menuDao.insert(com.opp.order.data.db.entity.MenuItemEntity(name = name, price = price, category = cat))
            }
